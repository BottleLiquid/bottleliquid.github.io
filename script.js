...
function startTeamChatListener(teamId) {
  ...
  teamChatCache = [];
  if (FB_READY) {
    // Use .limit() NOT .limitToLast() — limitToLast requires orderBy which
    // needs a composite index. limit() works on a simple where() query alone.
    // We sort client-side by ts so messages always appear in the right order.
    teamChatUnsub = db.collection('team_messages')
      ...
      .onSnapshot(
        ...
        err => {
          ...
          showToast('Team chat error: ' + err.message);
        }
      );
  } else {
