...
function startTeamChatListener(teamId) {
  ...
  if (FB_READY) {
    ...
    // We sort client-side by ts after receiving docs.
    teamChatUnsub = db.collection('team_messages')
      .where('teamId','==',teamId)
      .limitToLast(100)
      .onSnapshot(
        s => {
          teamChatCache = s.docs.map(d=>d.data()).sort((a,b)=>a.ts-b.ts);
          renderTeamChat();
        },
        err => {
          console.error('Team chat listener error:', err);
          // Fallback: try a simple get without orderBy
          db.collection('team_messages').where('teamId','==',teamId).get()
            .then(snap => {
              teamChatCache = snap.docs.map(d=>d.data()).sort((a,b)=>a.ts-b.ts);
              renderTeamChat();
            }).catch(e => console.error('Team chat fallback failed:', e));
        }
      );
  } else {
    const poll = () => { teamChatCache = (JSON.parse(localStorage.getItem('lt_team_chat')||'[]')).filter(m=>m.teamId===teamId).sort((a,b)=>a.ts-b.ts); renderTeamChat(); };
    poll(); teamChatUnsub = setInterval(poll, 2500);
  }
}
...
function renderTeamUpgrades() {
  ...
  list.innerHTML = `
    <div style="margin-bottom:15px;padding:10px;background:rgba(255,170,0,.05);border:1px solid rgba(255,170,0,.1);border-radius:6px;font-size:.85rem">
      Team Treasury: <span style="color:var(--ok);font-weight:700">${teamCache.treasury || 0} 🧢</span>
    </div>
    ${TEAM_UPGRADES.map(u => {
      const owned = upgrades.includes(u.id);
      const canBuy = !owned && (!u.requires || upgrades.includes(u.requires));
      return `
        <div class="team-upgrade-card ${owned ? 'owned' : ''}">
          ...
          ${owned ? '<div class="team-upgrade-status">Active</div>' : canBuy ? '' : u.requires ? '<div class="team-upgrade-locked">Requires ' + TEAM_UPGRADES.find(x=>x.id===u.requires).name + '</div>' : ''}
        </div>
      `;
    }).join('')}
  `;
...
async function createTeam() {
  ...
  await dbUpdateUser(getU(), { coins: UC.coins, teamId: teamId, teamRank: 'president' });
  refreshCoins();
...
async function joinTeam(teamId) {
  ...
  UC.teamId = teamId;
  UC.teamRank = 'member';
  await dbUpdateUser(getU(), { teamId: teamId, teamRank: 'member' });
...
async function disbandTeam() {
  ...
  for (const m of members) {
    await dbUpdateUser(m.username, { teamId: null, teamRank: null });
  }
