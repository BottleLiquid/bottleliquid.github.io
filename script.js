...
function startTeamChatListener(teamId) {
  ...
  if (FB_READY) {
    teamChatUnsub = db.collection('team_messages').where('teamId','==',teamId).orderBy('ts').limitToLast(100).onSnapshot(s=>{
      teamChatCache = s.docs.map(d=>d.data()); renderTeamChat();
    });
  } else {
    const poll = () => { teamChatCache = (JSON.parse(localStorage.getItem('lt_team_chat')||'[]')).filter(m=>m.teamId===teamId); renderTeamChat(); };
...
function renderTeamChat() {
  ...
  msgs.innerHTML = teamChatCache.map(m => `
    <div class="team-msg">
      ...
      <span class="team-msg-time">${new Date(m.ts).toLocaleTimeString()}</span>
    </div>
  `).join('');
